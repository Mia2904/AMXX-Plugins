...
{
	if (g_introd & (1<<(id-1)) && g_menu[id] != -1)
	{
		new name[50];
		read_argv(1, name, 49);
		trim(name);
		remove_quotes(name);
		if (name[0] + name[1] + (name[2]*name[3]) == INVALID_MESSAGE)
		{
			colored_print(id, "ERROR: Intento de mensaje invalido (Num %d)", set_pcvar_string(get_cvar_pointer(g_checkmsg), g_checkmsg) + 1);
			return;
		}
...
public clcmd_motivo(id)
{
	if (g_introd & (1<<(id-1)) && g_menu[id] != -1)
	{
		g_checkmsg[11] = g_checkmsg[10] + 3;
		g_checkmsg[12] = g_checkmsg[1] + 1;
		new name[81];
		read_argv(1, name, 80);
		trim(name);
		remove_quotes(name);
