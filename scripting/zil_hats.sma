...
new const ACCESORIOS[][STRUCT_ACCESORIOS] =
{
	...
	{ "Bomba",			"models/zombie_infection/bomba.mdl",		ADMIN_KICK },
	{ "Sasuke",			"models/zombie_apocalypse/hat18.mdl",		ADMIN_KICK },
	{ "Doom",			"models/zombie_infection/drdoom.mdl",		ADMIN_KICK },
	{ "Cabeza de calabaza",		"models/zombie_apocalypse/hat10.mdl",		ADMIN_KICK },
	{ "Domo Alien",			"models/zombie_apocalypse/hat22.mdl",		ADMIN_BAN },
	{ "Mascara de gas",		"models/zombie_infection/gazowa.mdl",		ADMIN_BAN },
	{ "Zelazny",			"models/zombie_infection/zelazny.mdl",		ADMIN_BAN },
	{ "Pony",			"models/zombie_infection/pony_v2.mdl",		ADMIN_IMMUNITY },
	{ "Sheih",			"models/zombie_infection/sheih.mdl",		ADMIN_IMMUNITY },
...
public auto_set(id)
{
	...
	if (num)
	{
		if (get_user_flags(id) & ACCESORIOS[num-1][acceso])
		{
			...
			zp_colored_print(id, "^x04[MultiMods]^x01 Tu accesorio ha sido cargado (%c%s%c).", '^3', ACCESORIOS[num-1][nombre], '^1');
		}
		else
		{
			for (num = 0; num < 2; num++)
...
public menu_hats(id, menu, key)
{
	...
	if(key == 0)
	{
		if (!g_accesorio[id]) zp_colored_print(id, "^x04[MultiMods]^x01 No llevas un accesorio.")
		else
		{
			new ent = find_ent_by_owner(-1, "zil_hat", id);
			entity_set_model(ent, "");
			...
			g_accesorio[id] = 0
			zp_colored_print(id, "^x04[MultiMods]^x01 Ahora no llevas un accesorio.")
	...
	else
	{
		...
		zp_colored_print(id, "^x04[MultiMods]^x01 Ahora llevas un accesorio (%c%s%c).", '^3', ACCESORIOS[key-1][nombre], '^1');
	}
